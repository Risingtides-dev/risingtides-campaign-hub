...
# --- sync_urls (explicit-list front door) ---------------------------------


def test_sync_urls_pushes_explicit_list_stamps_only_existing(db_env):
    # matched_videos has U1 only; we push [U1, U2] explicitly (U2 came from the
    # scrape diff and has no matched_videos row — expected).
    _seed(urls=(U1,))
    client = FakeClient(
        activations=[_act()],
        task_map={U1: ("SUCCESS", "c1", "s1"), U2: ("SUCCESS", "c2", "s2")},
    )
    report = sync_urls("camp", [U1, U2], config=_enabled_cfg(), client=client,
                       dry_run=False, sleep=lambda d: None)
    ...
    with _db.get_session() as s:
        # U1 had a matched_videos row -> stamped tracked.
        mv1 = s.query(MatchedVideo).filter_by(url=U1).first()
        assert mv1.tracked_at is not None
        assert mv1.tracked_by == "cobrand-sideload"
        # U2 had no matched_videos row -> none created.
        assert s.query(MatchedVideo).filter_by(url=U2).first() is None
        # Ledger is the record for both.
        assert s.query(CobrandSideloadTask).count() == 2
        assert s.query(CobrandSideloadTask).filter_by(url=U2).first().status == "SUCCESS"


def test_sync_urls_dry_run_makes_no_writes(db_env):
    _seed(urls=())  # no matched_videos at all
    client = FakeClient(activations=[_act()], task_map={})
    report = sync_urls("camp", [U1, U2], config=SideloadConfig(enabled=False),
                       client=client, dry_run=True, sleep=lambda d: None)
    assert report.dry_run is True
    assert report.uploaded == 0
    assert "dry-run" in report.message
...
def test_sync_urls_dedup_is_normalized(db_env):
    _seed(urls=())
    client = FakeClient(activations=[_act()], task_map={U1: ("SUCCESS", "c", "s")})
    sync_urls("camp", [U1], config=_enabled_cfg(), client=client,
              dry_run=False, sleep=lambda d: None)
    # Re-push the non-www variant of the same video; normalized dedup must skip it.
    u1_nowww = U1.replace("https://www.tiktok.com", "https://tiktok.com")
    client2 = FakeClient(activations=[_act()], task_map={})
    report = sync_urls("camp", [u1_nowww], config=_enabled_cfg(), client=client2,
                       dry_run=False, sleep=lambda d: None)
    assert report.skipped_already == 1
    assert report.uploaded == 0
    assert client2.uploaded is None
