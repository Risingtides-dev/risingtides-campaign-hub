...
export default function InternalGroupDetail() {
  ...
  if (!group) {
    return <p className="text-center text-rt-fg-tertiary py-20">Group not found.</p>
  }
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap items-start justify-between gap-3 mb-4">
        <div>
          <Link to="/internal" className="text-rt-magenta text-sm hover:underline flex items-center gap-1 mb-1">
      ...
      {/* Stat cards */}
      {statsLoading ? (
        <div className="flex items-center gap-2 text-rt-fg-tertiary text-xs mb-5">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-5">
          {[
            ...
            <div key={card.label} className="bg-rt-bg-card border border-white/8 rounded-[10px] p-4">
              <div className="text-rt-fg-tertiary text-xs font-semibold uppercase tracking-wide mb-1">{card.label}</div>
              <div className="font-bold text-rt-fg text-[22px]">{card.value}</div>
            </div>
          ))}
        </div>
      ) : null}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/8 bg-white/[0.03]">
              ...
              <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Posts</th>
              <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Views</th>
              <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Likes</th>
              <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">All-time</th>
          ...
          <tbody>
            {sortedMembers.map((username) => {
              ...
              return (
                <tr key={username} className="border-b border-white/5 last:border-b-0 hover:bg-white/[0.03]">
                  <td className="px-4 py-2.5">
                    <Link to={`/internal/${username}`} className="text-rt-magenta hover:underline">@{username}</Link>
                  </td>
                  <td className="px-4 py-2.5 text-right text-rt-fg-tertiary">{cStats?.posts ?? 0}</td>
                  <td className="px-4 py-2.5 text-right text-rt-fg-tertiary">{(cStats?.views ?? 0).toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-right text-rt-fg-tertiary">{(cStats?.likes ?? 0).toLocaleString()}</td>
                  <td className="px-4 py-2.5 text-right text-rt-fg">{allTime?.total_videos ?? 0} videos</td>
      ...
      {stats?.top_songs && stats.top_songs.length > 0 && (
        <div className="bg-rt-bg-card border border-white/8 rounded-[10px] overflow-hidden mb-5">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-white/8 bg-white/[0.03]">
                <th className="text-left px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Song</th>
                <th className="text-left px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Artist</th>
                <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Posts</th>
                <th className="text-right px-4 py-2.5 text-[12px] text-rt-fg-tertiary font-semibold uppercase tracking-wide">Views</th>
              </tr>
            </thead>
            <tbody>
              {stats.top_songs.map((song, i) => (
                <tr key={i} className="border-b border-white/5 last:border-b-0">
                  <td className="px-4 py-2.5 text-rt-fg">{song.song || "Unknown"}</td>
                  <td className="px-4 py-2.5 text-rt-fg-tertiary">{song.artist || "Unknown"}</td>
                  <td className="px-4 py-2.5 text-right text-rt-fg-tertiary">{song.posts}</td>
                  <td className="px-4 py-2.5 text-right text-rt-fg-tertiary">{song.views.toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
