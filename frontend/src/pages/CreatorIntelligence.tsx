...
function SoundFitView({ onPickCreator }: { onPickCreator: (a: string) => void }) {
  ...
  const sounds = useMemo(() => {
    ...
    return list.filter(
      (s) => s.artist.toLowerCase().includes(q) || s.song.toLowerCase().includes(q)
    )
  }, [soundsData, soundQuery])

  const maxFit = fit?.creators.length ? Math.max(...fit.creators.map((c) => c.fit_score)) : 1
      ...
      <div>
        {!picked && (
          <div className="flex h-full min-h-[300px] items-center justify-center rounded-2xl border border-dashed border-white/10 text-sm text-rt-fg-tertiary">
            <div className="text-center">
              ...
              Pick a sound to see who's most likely to break it.
            </div>
          </div>
        )}
        ...
        {picked && (
          <>
            ...
            {fit && (
              <div className="overflow-hidden rounded-2xl border border-white/8 bg-rt-bg-raised/60 backdrop-blur">
                <div className="grid grid-cols-[2.5rem_1fr_5rem_4.5rem_1fr] gap-3 border-b border-white/8 px-5 py-3 text-[10px] uppercase tracking-[0.16em] text-rt-fg-tertiary">
                  <span>#</span>
                  <span>Creator</span>
                ...
                {fit.creators.slice(0, 30).map((c, i) => (
                  <SoundFitRow key={c.account} c={c} i={i} maxFit={maxFit} onClick={() => onPickCreator(c.account)} />
                ))}
              </div>
            )}
          </>
...
function SoundFitRow({
  ...
  return (
    <button
      ...
      className="rt-rise grid w-full grid-cols-[2.5rem_1fr_5rem_4.5rem_1fr] items-center gap-3 border-b border-white/5 px-5 py-3 text-left transition-colors hover:bg-white/[0.03]"
    >
      <span className="rt-num text-sm text-rt-fg-tertiary">{i + 1}</span>
      <div className="min-w-0">
      ...
      <span className="rt-num text-right text-sm text-rt-fg-secondary">{c.viral_rate}%</span>
...
function ModeBtn({
  ...
  return (
    ...
    >
      {active && <span className="absolute inset-0 rounded-lg rt-gradient-bg opacity-90" />}
      <Icon className="relative z-10 h-4 w-4" />
