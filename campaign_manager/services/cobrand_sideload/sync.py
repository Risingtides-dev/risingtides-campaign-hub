...
Two front doors over one core (``_sync_resolved``):

- :func:`sync_campaign` — builds the URL batch from the Hub's ``matched_videos``
  table (the Scrape-Tasks queue semantics) and pushes it.
- :func:`sync_urls` — pushes an **explicit** list of URLs supplied by the caller
  (e.g. the ``rising-tides-scrape`` skill, which diffs TikTok against the
  TidesTracker and is the source of truth for *which* links to upload). The Hub
  cron / ``matched_videos`` is unreliable and misses internal-creator posts, so
  the scrape skill — not ``matched_videos`` — decides the set; this module only
  owns *how* the push happens.

Both resolve the activation, dedup against the ledger, bulk upload the untracked
links, poll the async group to completion, then record per-URL SUCCESS/FAILURE
into the ledger and (best-effort) mirror "mark whole campaign tracked" by
stamping ``MatchedVideo.tracked_at`` on success.

The entrypoints are gated by the ``COBRAND_SIDELOAD_ENABLED`` flag and are
...
def _normalize_url(u: str) -> str:
    """Canonicalize a TikTok URL for dedup (drop query, trailing slash, www).

    Mirrors ``rising-tides-scrape``'s ``lib.hub.normalize_tiktok_url`` so the two
    sides agree on identity when the skill hands us URLs.
    """
    if not u:
        return ""
    u = u.split("?")[0].rstrip("/")
...
def _preflight(config: SideloadConfig, dry_run: bool) -> None:
...
def sync_campaign(
    ...
    *,
    ...
    """Sideload one campaign's untracked ``matched_videos`` into co:brand.
    ...
    _preflight(config, dry_run)

    report = SyncReport(slug=slug, dry_run=dry_run)
    client = client or CobrandSideloadClient(config)

    # Load campaign + build the untracked batch (mirrors the Scrape Tasks queue
    # filters, incl. CAMP-42 pre-start-date exclusion).
    ...
    return _sync_resolved(
        report, report.promotion_id, camp_artist, camp_title, camp_id, batch,
        config=config, client=client, dry_run=dry_run,
        tracked_by=tracked_by, stamp_matched=True, sleep=sleep,
    )
...
def sync_urls(
    ...
    *,
    ...
    dry_run: bool = True,
    tracked_by: str = "cobrand-sideload",
    stamp_matched: bool = True,
    sleep: Callable[[float], None] = time.sleep,
) -> SyncReport:
    """Sideload an **explicit** list of URLs into a campaign's activation.

    The URL set is supplied by the caller (the ``rising-tides-scrape`` skill,
    source of truth for what to upload) rather than read from ``matched_videos``.

    Args:
        slug: campaign slug (used to resolve promotion_id -> activation).
        urls: the exact URLs to push (already filtered/diffed by the caller).
        dry_run: defaults to **True** here — resolve + build the batch, no writes.
        stamp_matched: best-effort. On SUCCESS, stamp ``tracked_at`` on a
            ``matched_videos`` row only if one already exists for the URL. URLs
            with no matched_videos row are expected and fine — the ledger is the
            authoritative record; rows are never created here.
        tracked_by / sleep: as in :func:`sync_campaign`.
    """
    config = config or SideloadConfig.from_env()
    _preflight(config, dry_run)
    ...
    with _db.get_session() as s:
        ...
        # Best-effort: map normalized URL -> existing matched_videos id so a
        # later SUCCESS can stamp tracked_at. Never creates rows.
        norm_to_mv: Dict[str, int] = {}
        if stamp_matched:
            for mv in (
                s.query(MatchedVideo).filter(MatchedVideo.campaign_id == camp.id).all()
            ):
                if mv.url:
                    norm_to_mv.setdefault(_normalize_url(mv.url), mv.id)

    # De-dup the incoming list by normalized URL; attach an mv id where one exists.
    batch: List[Dict] = []
    seen: set = set()
    for u in urls:
        if not u:
        ...
        if k in seen:
            continue
        seen.add(k)
        batch.append({"id": norm_to_mv.get(k), "url": u})

    return _sync_resolved(
        report, report.promotion_id, camp_artist, camp_title, camp_id, batch,
        config=config, client=client, dry_run=dry_run,
        tracked_by=tracked_by, stamp_matched=stamp_matched, sleep=sleep,
    )
...
def _sync_resolved(
    ...
    *,
    ...
    """Shared core: resolve activation -> dedup vs ledger -> (validate) ->
    upload -> poll -> record. ``batch`` is a list of ``{"id", "url"}`` dicts."""
    # Resolve activation_id (cache -> get_promotion).
    activation_id = _resolve_activation(client, promotion_id, artist, title, camp_id)
    report.activation_id = activation_id

    if not batch:
        report.message = "no urls to upload"
        return report

    # Map URL -> matched_video id (first wins after normalized de-dup upstream).
    url_to_mv: Dict[str, Optional[int]] = {}
    for b in batch:
        url_to_mv.setdefault(b["url"], b.get("id"))
    urls = list(url_to_mv.keys())

    # Idempotency: drop URLs already pushed (non-FAILURE) for this activation,
    # compared on the normalized form.
    ...
    # Optional validation (shape unconfirmed; off by default).
    ...
    # Bulk upload -> group handle. Record PENDING ledger rows.
    ...
    # Poll the async group until pending_count == 0.
    ...
    # Record per-task outcomes; mark successes tracked (best-effort).
    now = datetime.now()
    with _db.get_session() as s:
        for task in group.tasks:
            ...
            if task.status == STATUS_SUCCESS:
                report.succeeded += 1
                if stamp_matched and mvid is not None:
...
def _already_pushed_urls(activation_id: str, urls: List[str]) -> set:
    """URLs in ``urls`` that already have a non-FAILURE ledger row for this
    activation, compared on the normalized URL form."""
    if not urls:
        return set()
    norm_to_original = {_normalize_url(u): u for u in urls}
    pushed = set()
    with _db.get_session() as s:
        rows = (
            s.query(CobrandSideloadTask)
        ...
        for r in rows:
            if r.status == STATUS_FAILURE:
                continue
            original = norm_to_original.get(_normalize_url(r.url))
