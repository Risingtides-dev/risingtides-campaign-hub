...
- **Notion multi-source API fix** (2026-07-31) -- On 2026-07-28 Notion migrated the workspace to multi-source databases (both the CRM and Master Pages databases gained an empty second "New data source"), which makes every query from API version `2022-06-28` fail with HTTP 400. This silently killed BOTH Notion syncs: the 15-min Master Pages cron (286 consecutive failed runs, visible in `notion_sync_log`) and the CRM→campaign webhook (`POST /api/webhooks/notion/sync`), which swallowed the error and reported "no new campaigns" — it has in fact never created a campaign. Fix: `NOTION_VERSION` bumped to `2025-09-03`; new `resolve_data_source_id()` in `services/notion.py` resolves database ID → data source ID (first-listed source wins; pin explicitly via `NOTION_CRM_DATA_SOURCE_ID` / `NOTION_MASTER_PAGES_DATA_SOURCE_ID` if needed); both query paths now hit `/v1/data_sources/<id>/query`; `query_new_clients` logs failures instead of returning silently. Known limitation: the CRM sync only picks up entries with Pipeline Status = "Client", but Seeno's entries sit at "Lead" — the trigger condition is an open product decision. Separately: some external process (not in this repo) creates Hub campaigns from CRM Lead entries daily with `source='manual'` + `notion_page_id`; its identity is unconfirmed.
